...
function MigrateJob(){
	for ((i=0;i<=$LENGTH;i++))
	do
		curl -u ${USER}:${PWD} -X POST --data-binary "@$WORKSPACE/${arr_jobname[$i]}.xml" --header "Content-type: application/xml" ${TarURI}/createItem?name=${arr_jobname[$i]}
	done
}
...
do
	case $ARG in
		...
		l)
			...
			;;
		h|--help)
			echo "Usage: $0 /u/p/m/v/j/l \n"
			echo "-u username is needed \n"
			echo "-p password is needed \n"
			echo "-v viewname is needed \n"
			echo "-j need to be declared \n"
			echo "-l JenkinsURL is needed \n"
			echo "-m target jenkins url is needed \n"
