...
class exportvalue:
    def __init__(self,excelfile,sheetName=u'Sheet1'):
        self.excelfile = excelfile
        self.sheetName = sheetName

    def GetRowValue(self,hostip):
        excelobj = readExcel(self.excelfile)
        output = excelobj.excel_table_byname(by_sheetName=self.sheetName)
        for index in xrange(len(output)):
            for data in output[index]:
                if data == hostip:
                    ThIndex = output[index].index(hostip)
                    return output[index]
...
def main():
    m = exportvalue(u'quzhouZoneUserInfo.xlsx',sheetName=u'.....02')
    usage = "Usage: %prog [options] arg"
    parser = OptionParser(usage)
    #parser.add_option("-s","--search", type="str",
                      #help="--search: which is the relative information you want")
