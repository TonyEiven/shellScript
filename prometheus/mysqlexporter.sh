#!/bin/bash
### BEGIN INIT INFO
# Provides:          mysql_exporter
# Required-Start:    $all
# Required-Stop:     $remote_fs $syslog
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Short-Description: Start the mysql_exporter process
### END INIT INFO

# Command-line options that can be set in /data/mysql_exporter/.  These will override
# any config file values.
DEFAULT=/data/mysqld_exporter

# Process name ( For display )
NAME=mysqld_exporter

# Configuration file
CONFIG=/data/mysqld_exporter

# Max open files
OPEN_FILE_LIMIT=65536

# PID file for the daemon
PIDFILE=/data/mysqld_exporter/mysqld_exporter.pid
...
function start() {
    ...
    if [ -x $DEFAULT/$NAME ]; then
            $DEFAULT/$NAME --config.my-cnf="/etc/.my.cnf" --collect.slave_hosts --log.level=debug >> $DEFAULT/mysql_exporter.log 2>&1 &
    fi
    # Write PID to PIDFILE
	sleep 10
    result=$(pgrep -f mysqld_exporter)
    if [[ $result != "" ]];then
	echo $result > $PIDFILE
    fi
